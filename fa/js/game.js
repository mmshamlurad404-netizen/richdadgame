...
async function takeTurn() {
  ...
  // بازیکن بعدی — بازیکن ورشکسته را رد کن
  let guard = 0;
  do {
...
/* ---------------- معامله بازیکن با بازیکن ---------------- */
/* آیا خریدار هوش مصنوعی این خرید را می‌پذیرد؟ به‌صرفه بودن، بازگشت سرمایه و قیمت منطقی را می‌سنجد. */
...
/* آیا فروشنده هوش مصنوعی این قیمت را می‌پذیرد؟ فقط برای یک سود خوب نسبت به ارزش بازار. */
...
function transferAsset(seller, buyer, asset, price) {
  ...
  sfx.coin();
  log(`${buyer.name} ${asset.name} را از ${seller.name} به ${fmt(price)} می‌خرد.`);
  saveGame();
  renderAll();
}
